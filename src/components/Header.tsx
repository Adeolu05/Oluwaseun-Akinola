    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ease-out
          ...
          ${
            isScrolled
              ? 'border-b border-muted-sage/35 bg-pure-white/92 backdrop-blur-lg shadow-[0_4px_24px_-4px_rgba(16,42,67,0.08)] pb-3.5 md:pb-4'
              : 'border-b border-muted-sage/15 bg-pure-white/85 backdrop-blur-md pb-4 md:border-b-transparent md:bg-transparent md:backdrop-blur-none md:pb-5'
          }`}
      >
        <div
        ...
        >
          <a
            href="#top"
            className="min-w-0 shrink text-left text-base sm:text-lg md:text-xl font-serif font-medium text-navy-blue leading-tight hover:text-deep-green transition-colors py-1"
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <a
              href="#contact"
              className="px-6 py-2.5 rounded-full bg-deep-green text-pure-white text-sm font-medium hover:bg-deep-green-hover transition-all shadow-md hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-deep-green"
            >
              Invite Me
            </a>
          </nav>
          ...
          <>
            ...
            >
              ...
              <ul className="flex flex-col gap-0.5 flex-1 overflow-y-auto overscroll-contain -mr-1 pr-1">
                {navLinks.map((link) => (
                  <li key={link.name}>
                    <a
                      href={link.href}
                      className="group flex items-center justify-between gap-3 rounded-xl px-3 py-3.5 text-[17px] font-medium text-charcoal hover:bg-warm-ivory hover:text-navy-blue transition-colors border border-transparent hover:border-muted-sage/25"
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      <span>{link.name}</span>
                      <ChevronRight className="size-5 text-soft-gold/70 group-hover:text-soft-gold group-hover:translate-x-0.5 transition-transform shrink-0" aria-hidden />
                    </a>
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-4 pt-5 border-t border-muted-sage/25">
                <a
                  href="#contact"
                  className="flex w-full items-center justify-center rounded-2xl bg-deep-green px-6 py-4 text-base font-semibold text-pure-white shadow-md hover:bg-deep-green-hover transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-deep-green"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Invite Me to Speak
                </a>
